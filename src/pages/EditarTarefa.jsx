...
function EditarTarefa() {
  // Extrair ID na rota dinâmica
  const { id }= useParams(); // useParams vai dar um objeto com os parâmetros da rota.
  
  const {
    register,
    handleSubmit,
    formState: { errors },
    reset // serve tanto para limpar, quanto para preencher
  } = useForm();
  ...
  function carregarDado() {
    getTarefa(id).then((tarefa) => {
      if(tarefa) { // se existir a tarefa,
        ...
        reset(tarefa); // passando um objeto ele pega os dados dele e joga nos campos do formulário, desde que tenham os mesmo nome.
      } else {
        navigate("/tarefas"); // se não existe, volta para página de tarefas
      }
    });
  }

  function atualizarTarefa(data) { // quando o usuário atualizar a tarefa o onSubmit chama essa função aqui
